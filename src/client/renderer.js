...
export const Resources = {
  ...
  load() {
    for (let i = 0; i < 12; i++) {
      const img = new Image(); img.src = `img/food${i}.png`; this.foodImages[i] = img;
    }
  ...
  getPlayerImage(level) {
    if (!this.playerImages[level]) { const img = new Image(); img.src = `img/lv${level}.png`; this.playerImages[level] = img; }
    return this.playerImages[level];
  },
  getWeaponImage(level) {
    if (!this.weaponImages[level]) { const img = new Image(); img.src = `img/weapon${level}.png`; this.weaponImages[level] = img; }
...
export const Renderer = {
  ...
  addDeathParticles(x, y, radius) {
    ...
    for (let i = 0; i < numParticles; i++) {
      ...
      this.particles.push({
        ...
        life: 1.0, decay: Math.random() * 0.03 + 0.015, size: Math.random() * (radius * 0.35) + 3,
  ...
  renderBackground() {
    if (!Resources.offscreenCanvas || Resources.offscreenCanvas.width !== CONFIG.MAP_WIDTH || Resources.offscreenCanvas.height !== CONFIG.MAP_HEIGHT) {
      ...
      Resources.offscreenCanvas.width = CONFIG.MAP_WIDTH; Resources.offscreenCanvas.height = CONFIG.MAP_HEIGHT;
    ...
    if (Resources.mapPatternReady && Resources.mapBgImg.complete && Resources.mapBgImg.naturalHeight !== 0) {
      Resources.offscreenCtx.save(); Resources.offscreenCtx.imageSmoothingEnabled = false;
      Resources.offscreenCtx.fillStyle = Resources.mapPattern;
      Resources.offscreenCtx.fillRect(0, 0, CONFIG.MAP_WIDTH, CONFIG.MAP_HEIGHT); Resources.offscreenCtx.restore();
    } else {
      Resources.offscreenCtx.fillStyle = "#000"; Resources.offscreenCtx.fillRect(0, 0, CONFIG.MAP_WIDTH, CONFIG.MAP_HEIGHT);
    }
  },
  updateUI() {
  ...
  getInterpolatedState() {
    ...
    let t = 0;
    if (older !== newer && newer.time !== older.time) t = Math.max(0, Math.min(1, (renderTime - older.time) / (newer.time - older.time)));
    const interpFood = newer.food.map((f) => { const prev = older.food.find((o) => o.id === f.id) || f; return this.lerpObj(prev, f, t); });
    const interpPlayers = {};
  ...
  drawShield(x, y, radius, justRespawned) {
    ...
    if (shieldTimeLeft > 0) {
      ...
      ctx.lineWidth = 4; ctx.strokeStyle = "rgba(0, 255, 255, 0.8)"; ctx.stroke();
      ctx.restore();
    }
  },
  ...
  draw(dtMultiplier = 1) {
    ...
    if (Resources.offscreenCanvas) {
      ctx.save(); ctx.beginPath(); ctx.rect(0, 0, CONFIG.MAP_WIDTH, CONFIG.MAP_HEIGHT); ctx.clip();
      ctx.imageSmoothingEnabled = false; ctx.drawImage(Resources.offscreenCanvas, 0, 0); ctx.restore();
