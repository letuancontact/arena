...
try { isMutedGlobal = localStorage.getItem("evowar_muted") === "true"; } catch (e) {}

// HỆ THỐNG ÂM THANH
const Sound = {
  ctx: null, lastPlay: {}, noiseBuffer: null, isMuted: isMutedGlobal,
  init() {
    if (!this.ctx) { 
  ...
  toggleMute() {
    ...
    try { localStorage.setItem("evowar_muted", this.isMuted); } catch(e){}
    return this.isMuted;
  },
  play(type) {
...
if (nameInput) {
    ...
    try { nameInput.value = localStorage.getItem("evowar_name") || ""; } catch(e){}
}
...
const Network = {
  ...
  connect() { 
    try {
        ...
        this.ws.onclose = () => { 
            ...
            if (respawnBtn) { respawnBtn.innerText = "MẤT KẾT NỐI"; respawnBtn.disabled = true; }
    ...
    try {
        ...
        if (data.type === "state") {
          ...
          if (me) {
            ...
            // --- FIX 3: NHÂN VẬT TO BẰNG BOT ---
            // Bắt buộc lấy kích thước trực tiếp từ Server trả về (me.radius)
            ...
            // XỬ LÝ LÚC BỊ GIẾT: HIỂN THỊ MÀN HÌNH CHẾT 3 CỘT
            if (!prevDead && me.isDead) {
              if (uiLayer) { 
              ...
              if(respawnBtn) respawnBtn.innerText = `HỒI SINH SAU: ${left}S`;
          ...
          for (const p of data.players || []) {
            if (p.isDead && !GameState.prevPlayerDeadState[p.id]) {
                ...
                if (p.killerId === GameState.playerId && p.id !== GameState.playerId) {
                    if(Renderer.addFloatingText) {
                        const xpGained = Math.floor((p.score || 0) * (CONFIG.KILL_SCORE_MULTIPLIER_HUD || 0.5));
                        Renderer.addFloatingText(p.x, p.y - 12, `+${xpGained} XP`, "#00ff66", 14); 
                        Renderer.addFloatingText(p.x, p.y + 12, "KILL!", "#ff3333", 16);        
                    }
                    Sound.play('kill'); GameState.freezeUntil = Date.now() + 40; 
                    if(Camera.addShake) Camera.addShake(15); 
                    if(FX && FX.spawnHitSparks) FX.spawnHitSparks(p.x, p.y);
                }
            }
            if (p.id === GameState.playerId) { 
                ...
                if (prevLevel && p.level > prevLevel && !p.isDead) {
                    if(Renderer.addLevelUpEffect) Renderer.addLevelUpEffect(p.x, p.y, p.radius);
                    if(Renderer.addFloatingText) Renderer.addFloatingText(p.x, p.y - p.radius - 20, "LEVEL UP!", "#00ffff", 18); 
                }
            }
  ...
  sendPositionUpdate(forceUpdate = false) {
    try {
        ...
        if (!forceUpdate && Date.now() - (GameState.lastSentTime || 0) < (CONFIG.CLIENT_SEND_INTERVAL || 50)) return;
        
        const dAngle = Math.abs(GameState.mouseAngle - (GameState.lastSentAngle || 0));
        ...
        if (forceUpdate || dAngle > (CONFIG.ANGLE_SEND_THRESHOLD || 0.05) || rightMouseChanged || movingChanged) {
  ...
  setup() {
    ...
    canvas.addEventListener("touchstart", (e) => {
      ...
      for(let i=0; i<e.changedTouches.length; i++) {
        ...
        if (isPointInCircle(tx, ty, w - 75, h - 75, 60)) {
          ...
          if (!GameState.joystick) GameState.joystick = {};
    ...
    canvas.addEventListener("touchmove", (e) => {
      ...
      for(let i=0; i<e.changedTouches.length; i++) {
        ...
        if (GameState.joystick && GameState.joystick.active && t.identifier === GameState.joystick.id) {
    ...
    canvas.addEventListener("touchend", (e) => {
      ...
      for(let i=0; i<e.changedTouches.length; i++) {
        const t = e.changedTouches[i];
        if (GameState.joystick && GameState.joystick.active && t.identifier === GameState.joystick.id) { GameState.joystick.active = false; GameState.isMoving = false; Network.sendPositionUpdate(true); }
    ...
    if(Renderer.resizeCanvas) window.addEventListener("resize", Renderer.resizeCanvas);
  }
};
...
function updatePhysics(dtMultiplier) {
  ...
  const attackDuration = (CONFIG.BASE_ATTACK_DURATION || 300) + (GameState.clientLevel * (CONFIG.ATTACK_DURATION_PER_LEVEL || 5));
  if (GameState.isAttacking && Date.now() - GameState.attackTime < attackDuration) {
    ...
    const speed = (GameState.getSpeedByLevel ? GameState.getSpeedByLevel(GameState.clientLevel) : 4) * (GameState.rightMouseDown && GameState.clientXp > 0 ? (CONFIG.SPRINT_MULTIPLIER || 1.5) : 1);
    GameState.clientX += Math.cos(GameState.mouseAngle) * speed * dtMultiplier;
    GameState.clientY += Math.sin(GameState.mouseAngle) * speed * dtMultiplier;
  }
  
  GameState.clientX = Math.max(GameState.clientRadius, Math.min((CONFIG.MAP_WIDTH || 3000) - GameState.clientRadius, GameState.clientX));
  GameState.clientY = Math.max(GameState.clientRadius, Math.min((CONFIG.MAP_HEIGHT || 3000) - GameState.clientRadius, GameState.clientY));
...
function main() {
  try {
      ...
      Camera.targetZoom = Camera.currentZoom || 1;
